...
class type t = object
  ...
  (** [add_clause solver cs] adds a boolean clause to the solver. Subsequent
      calls to {!solve} will need to satisfy this clause. *)
  ...
  (** Checks the satisfiability of the clauses added so far to the solver.
      ...
      @param should_stop
        ...
        of "steps" done since last call. The exact notion of step is not
        defined, but is guaranteed to increase regularly. The function should
        return [true] if it judges solving must stop (returning [Unknown]),
        [false] if solving can proceed.
      @param on_exit functions to be run before this returns *)

  method last_res : Check_res.t option
  (** Returns the result of the last call to {!solve}, if the logic statee has
      not changed (mostly by asserting new clauses). *)
