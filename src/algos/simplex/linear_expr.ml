...
module Make (C : COEFF) (Var : VAR) = struct
  ...
  module Comb = struct
    ...
    let as_singleton m =
      if is_empty m then
        ...
        if is_empty (Var_map.remove x m) then
          Some (c, x)
        else
          None
      )
    ...
    let pp_pair = Fmt.(pair ~sep:(return "@ * ") C.pp Var.pp)
  ...
  module Expr = struct
    type t = { const: C.t; comb: Comb.t }

    let[@inline] const e = e.const
    let[@inline] comb e = e.comb

    let compare e e' =
      CCOrd.(C.compare e.const e'.const <?> (Comb.compare, e.comb, e'.comb))
    ...
    module Infix = struct
      ...
      let ( * ) c e =
        if C.equal C.zero c then
          ...
          make Comb.(c * e.comb) C.(c * e.const)
    end
  ...
  module Constr = struct
    ...
    let compare c c' =
      CCOrd.(poly c.op c'.op <?> (Expr.compare, c.expr, c'.expr))
    ...
    let pp out c = Format.fprintf out "(@[%a@ %a 0@])" Expr.pp c.expr pp_op c.op
    let op t = t.op
    let expr t = t.expr
