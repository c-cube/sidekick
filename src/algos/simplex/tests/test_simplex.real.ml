...
module Var = struct
  ...
  let rand n : t QC.arbitrary =
    QC.make ~print:(Fmt.to_string pp) @@ QC.Gen.(0 -- n)
...
let rand_q_with u l : Q.t QC.arbitrary =
  ...
  let qc =
    QC.map
      ~rev:(fun q -> Q.num q, Q.den q)
      (fun (x, y) -> Q.make x y)
      (QC.pair n1 n2)
  in
  (* avoid [undef] when shrinking *)
  let shrink q yield =
    unwrap_opt_ "no shrinker" qc.QC.shrink q (fun x ->
        if Q.is_real x then yield x)
  in
...
module Step = struct
  ...
  let pp_le out le =
    let pp_pair out (n, x) =
      if Q.equal Q.one n then
        Var.pp out x
      else
        Fmt.fprintf out "%a . %a" Q.pp_print n Var.pp x
    in
    Fmt.fprintf out "(@[%a@])" Fmt.(list ~sep:(return " +@ ") pp_pair) le

  let pp_ out = function
    | S_new_var v -> Fmt.fprintf out "(@[new-var %a@])" Var.pp v
    | S_define (v, le) ->
      Fmt.fprintf out "(@[define %a@ := %a@])" Var.pp v pp_le le
    | S_leq (x, n) ->
      Fmt.fprintf out "(@[upper %a <= %a@])" Var.pp x Q.pp_print n
    | S_lt (x, n) -> Fmt.fprintf out "(@[upper %a < %a@])" Var.pp x Q.pp_print n
    | S_geq (x, n) ->
      Fmt.fprintf out "(@[lower %a >= %a@])" Var.pp x Q.pp_print n
  ...
  let well_formed (l : t list) : bool =
    let rec aux vars = function
      | [] -> true
      | S_new_var v :: tl -> (not (List.mem v vars)) && aux (v :: vars) tl
  ...
  let shrink_step self =
    ...
    match self with
    | S_new_var _ | S_leq _ | S_lt _ | S_geq _ | S_gt _ -> QC.Iter.empty
    | S_define (x, le) ->
      let open QC.Iter in
      let* le = S.list le in
      if List.length le >= 2 then
        return (S_define (x, le))
      else
        empty
...
let add_step simplex (s : Step.t) : unit =
  ...
  | Step.S_define (x, le) -> Spl.define simplex x le
...
let prop_sound ?(inv = false) pb =
  ...
  | Sat subst ->
    let get_val v =
      try Spl.V_map.find v subst with Not_found -> assert false
    in
    ...
    let check_step s =
      (try
         ...
         | Step.S_define (x, le) ->
           ...
           if Q.(v_x <> v_le) then
             failwith
               (spf "bad def (X_%d): val(x)=%s, val(expr)=%s" x (q_dbg v_x)
                  (q_dbg v_le))
         | Step.S_lt (x, n) ->
           let v_x = get_val x in
           if Q.(v_x >= n) then
             failwith (spf "val=%s, n=%s" (q_dbg v_x) (q_dbg n))
         | Step.S_leq (x, n) ->
           let v_x = get_val x in
           if Q.(v_x > n) then
             failwith (spf "val=%s, n=%s" (q_dbg v_x) (q_dbg n))
         ...
         | Step.S_geq (x, n) ->
           ...
           if Q.(v_x < n) then
             failwith (spf "val=%s, n=%s" (q_dbg v_x) (q_dbg n))
       with e ->
         QC.Test.fail_reportf "proof_rule failed: %a@.exn:@.%s@." Step.pp_ s
  ...
  | Spl.Unsat cert ->
    ...
    true
  | exception Spl.E_unsat _cert -> true
  ...
  [
    ...
    ( "n-bnd",
      Step.(
        List.fold_left
          (fun n -> function
            | S_leq _ | S_lt _ | S_geq _ | S_gt _ -> n + 1
            | _ -> n)
          0) );
    ( "n-vars",
      Step.(
        List.fold_left
          (fun n -> function
            | S_define _ | S_new_var _ -> n + 1
            | _ -> n)
          0) );
  ]

let enable_stats =
  match Sys.getenv_opt "TEST_STAT" with
  | Some ("1" | "true") -> true
  | _ -> false

let set_stats_maybe ar =
  if enable_stats then
    QC.set_stats steps_stats ar
  else
    ar
...
let check_sound =
  let ar =
    Step.(rand_for 0 300)
    |> QC.set_collect (fun pb ->
           if check_steps pb then
             "sat"
           else
             "unsat")
    |> set_stats_maybe
...
let check_invariants =
  let ar =
    ...
    |> set_stats_maybe
  in
  QC.Test.make ~long_factor:10 ~count:500 ~name:"simplex2_invariants" ar
    prop_invariants
...
let check_backtrack =
  let ar =
    ...
    |> set_stats_maybe
  in
  QC.Test.make ~long_factor:10 ~count:200 ~name:"simplex2_backtrack" ar
    prop_backtrack
...
let check_scalable =
  ...
  let ar =
    ...
    |> set_stats_maybe
  in
  QC.Test.make ~long_factor:2 ~count:10 ~name:"simplex2_scalable" ar prop

let props = [ check_invariants; check_sound; check_backtrack; check_scalable ]
...
module Reg = struct
  ...
  let t1 =
    let l =
      [
        S_new_var 1; S_gt (1, qstr "2630/16347"); S_leq (1, qstr "26878/30189");
      ]
    in
    reg_prop_sound "t1" l
    ...
    let l =
      [
        ...
        S_define
          ( 4,
            [
              qstr "-22841/11339", 0; qstr "5792/491", 1; qstr "-48819/5089", 3;
            ] );
      ]
    in
    reg_prop_sound "t3" l
  ...
  let t4_snd =
    let l =
      [
        ...
        S_define (4, [ qstr "-26393/10015", 2; qstr "-12730/2099", 3 ]);
        S_new_var 6;
        S_define
          ( 7,
            [
  ...
  let t7 =
    let l =
      [
        S_new_var 1; S_leq (1, qstr "32908/13565"); S_gt (1, qstr "92197/15966");
      ]
    in
...
let tests = "simplex", List.flatten [ Reg.tests ]
