...
module Make (A : ARG) : Sigs.S = struct
  open struct
    (* proof rules *)
  ...
  module Expl = struct
    ...
    let rec pp out (e : explanation) =
      ...
      | E_merge_t (a, b) ->
        Fmt.fprintf out "(@[<hv>merge@ @[:n1 %a@]@ @[:n2 %a@]@])" Term.pp_debug
          a Term.pp_debug b
      | E_theory (t, u, es, _) ->
        Fmt.fprintf out "(@[th@ :t `%a`@ :u `%a`@ :expl_sets %a@])"
          Term.pp_debug t Term.pp_debug u
          (Util.pp_list
  ...
  type t = {
    ...
    on_is_subterm:
      (t * E_node.t * Term.t, Handler_action.t list) Event.Emitter.t;
  ...
  module Debug_ = struct
    ...
    let pp out (self : t) : unit =
      ...
      in
      let pp_n out n =
        Fmt.fprintf out "(@[%a%a%a%a@])" Term.pp_debug n.n_term pp_root n
          pp_next n pp_expl n
  ...
  *)
  ...
  module Expl_state = struct
    ...
    (* proof of [\/_i ¬lits[i]] *)
    let proof_of_th_lemmas (self : t) (proof : Proof_trace.t) :
        Proof_term.step_id =
  ...
  let create ?(stat = Stat.global) ?(size = `Big) (tst : Term.store)
      (proof : Proof_trace.t) : t =
    let size =
      match size with
      | `Small -> 128
      | `Big -> 2048
