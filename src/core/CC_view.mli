...
(** A view of a term fron the point of view of a congruence closure.
    ...
    - ['ts] is the type of sequences of terms (arguments of function
      application) *)
type ('f, 't, 'ts) t =
  | Bool of bool
  | App_fun of 'f * 'ts
...
val map_view :
  ...
  ('a, 'c, 'e) t ->
  ('b, 'd, 'f) t
(** Map function over a view, one level deep. Each function maps over a
    different type, e.g. [f_t] maps over terms *)
