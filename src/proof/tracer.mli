(** Proof traces.
    ...
    A proof trace is a log of all the deductive reasoning steps made by the SMT
    solver and other reasoning components. It essentially stores a DAG of all
    these steps, where each step points (via {!step_id}) to its premises. *)
...
(** A proof tracer.
    ...
    A proof tracer builds a log of all deductive steps taken by the solver, so
    we can later reconstruct a certificate for proof-checking.

    Each step in the proof trace should be a {b valid lemma} (of its theory) or
    a {b valid consequence} of previous steps. *)
class type t = object
  ...
  (** Forget a step that won't be used in the rest of the trace. Only useful for
