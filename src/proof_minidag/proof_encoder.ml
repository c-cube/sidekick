(** Encode sidekick [Pterm.t] proof steps into minidag nodes.
    ...
    Targeting the granite [ext-proof-fmt.json] command vocabulary: p.hyp p.res
    p.eq rs.a rs.r eq.u eq.c seq clause

    Sidekick-specific oracle steps use [sk.*] commands: sk.cc_conflict
    sk.sat_rup sk.bool_ax sk.r1 sk.p1 sk.lra sk.preprocess sk.rw-clause sk.sorry

    Step IDs ([Proof.Step.id] = [Entry_id.t] = int) are the minidag byte offset
    of the corresponding proof node — no separate table needed. *)

open Sidekick_proof
module E = Sidekick_minidag.Encode
open Term_encoder

type t = { te: Term_encoder.t; tst: Term.store }
...
let emit_hyp_lits self lits = emit_hyp self (List.map (encode_lit' self) lits)
...
let rec encode_rule self (r : Pterm.rule_apply) : E.offset =
  ...
  | "sat.input" -> emit_hyp_lits self lit_args
  | "sat.rc" ->
  ...
  | "core.true" -> emit_hyp self [ encode_term' self (Term.true_ self.tst) ]
  | "lra.lemma" ->
    let lit_offs = List.map (encode_lit' self) lit_args in
    nd self "sk.lra" (fun e -> List.iter (E.ref e) lit_offs)
  | name when String.length name >= 5 && String.sub name 0 5 = "bool." ->
