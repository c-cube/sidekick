(** Encode sidekick [Term.t] values into minidag nodes.
    ...
    Uses the term command vocabulary from [ext-term.json]: t.type t.v t.bv t.c
    t.@ t.\ t.-> cst

    Constants are encoded with the [cst] command (name + type). Terms are
    memoized: [Term.Tbl] for terms, [Const_tbl] for constants. *)
