module Buf = struct
  ...
  let create ?(cap = 16) () : t = { len = 0; b = Bytes.create cap }
  ...
  let[@inline] ensure_size_ (self : t) (new_len : int) : unit =
    if new_len > Bytes.length self.b then resize_ self new_len
...
module Writer = struct
  type t = { write: bytes -> int -> int -> unit; write_buf: Buffer.t -> unit }

  let nop_ _ = ()
  let dummy : t = { write = (fun _ _ _ -> ()); write_buf = (fun _ -> ()) }
...
module Reader = struct
  ...
  let[@inline] next (self : t) ~yield ~finish = self.read ~yield ~finish
